...
by `exp(-deficit / 8)` — where `deficit` is how far that track's popularity sits below its **own
playlist's** 86th percentile, clamped to 45 points. One round in eleven skips the weighting
...
deep cuts are ~280x rarer than its singles without anyone tuning a knob per playlist. Nothing becomes
...
there costs the head a fraction of its share instead of flattening the whole curve.
